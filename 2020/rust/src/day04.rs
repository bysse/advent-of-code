...
fn has_all_fields(record: &&HashMap<String, String>) -> bool {
    record.len() >= 7 && record.len() <= 8 &&
        record.contains_key("byr") && record.contains_key("iyr") &&
        record.contains_key("eyr") && record.contains_key("hgt") &&
        record.contains_key("hcl") && record.contains_key("ecl") &&
...
pub fn run() {
    ...
    let has_all_fields = records.iter()
        .filter(has_all_fields)
        .collect::<Vec<&HashMap<String, String>>>();
    println!("A: {}", has_all_fields.len());

    println!("B: {}", has_all_fields.iter()
