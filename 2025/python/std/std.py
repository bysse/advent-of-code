...
def tdlr_2d(x, y, width, height, include_center=False):
...
def display_2d(data, colors=None, output="data.png", scale=3, load=True, enable=True):
    if not enable:
        return
    if not colors:
        colors = {
    ...
    for y in range(height):
        ...
        for x in range(len(line)):
            if scale == 1:
                pixels[x, y] = color_for(line[x])
            else:
                for dy in range(scale):
                    for dx in range(scale):
                        pixels[x * scale + dx, y * scale + dy] = color_for(line[x])
